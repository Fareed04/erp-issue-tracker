...
import { AlertCircle, Clock, CheckCircle2, CircleDashed, MoreVertical, LayoutGrid } from 'lucide-react';
            ...
            <div key={group.id} className={clsx("flex flex-col gap-4", swimlaneBy === 'none' ? 'h-full' : 'shrink-0')}>
              ...
              <div className={clsx("flex gap-4 lg:gap-6", swimlaneBy === 'none' ? 'h-full' : 'shrink-0')}>
                {COLUMNS.map(column => {
                  ...
                  return (
                    <div key={column.id} className={clsx(
                      "w-[280px] lg:w-80 flex flex-col bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 shrink-0",
                      ...
                      <div className={clsx("p-4 space-y-3", swimlaneBy === 'none' ? 'flex-1 overflow-y-auto' : 'min-h-[120px]')}>
